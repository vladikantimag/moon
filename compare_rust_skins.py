...
def load_moon_prices(path: str) -> Dict[str, float]:
    """
    Parse moon.txt:
        Name
        99.54 $
        product
    and build mapping name -> best price on Moon.
    """
    with open(path, "r", encoding="utf-8") as f:
    ...
    while i + 1 < len(lines):
        ...
        current = moon_prices.get(key)
        # Keep the highest Moon price if multiple entries
...
def main() -> None:
    ...
    moon_prices = load_moon_prices(MOON_PATH)
    ...
    # Gross and net Moon totals (after commission)
    total_moon_gross = sum(
        float(e["moon_unit"]) * int(e["count"]) for e in can_sell_grouped.values()
    )
    total_moon_net = total_moon_gross * (1 - MOON_FEE_PERCENT) - MOON_FEE_FIXED
    if total_moon_net < 0:
        total_moon_net = 0.0
    ...
    # Output
    print("\nItems you can sell on Moon:")
    can_sell_sorted = sorted(can_sell_grouped.values(), key=lambda e: float(e["moon_unit"]) * int(e["count"]), reverse=True)
    for e in can_sell_sorted:
        name = str(e["name"])
        count = int(e["count"])
        moon_unit = float(e["moon_unit"])
        steam_unit = float(e["steam_sum"]) / count if count > 0 else 0.0
        ratio = (moon_unit / steam_unit * 100) if steam_unit > 0 else 0.0
        prefix = f"{count}x " if count > 1 else ""
        print(f"{prefix}{name}: {moon_unit:.2f} / {steam_unit:.2f} = {ratio:.2f}%")
    print("---")
    overall_ratio = (
        (total_moon_net / total_steam_for_can * 100) if total_steam_for_can > 0 else 0.0
    )
    print(
        f"Total: {total_moon_net:.2f} / {total_steam_for_can:.2f} = {overall_ratio:.2f}% / {can_sell_count} items"
    )
    print()

    print("Items you can't sell on Moon:")
    ...
    if desired_percent > 0 and can_sell_grouped:
        print(f"Items you can sell on Moon with {desired_percent:.2f}% or higher:")
        filtered: List[Tuple[str, int, float, float, float]] = []
        # (name, count, moon_unit, steam_unit, ratio)
        for e in can_sell_grouped.values():
            ...
            if steam_unit <= 0:
                continue
            ratio = moon_unit / steam_unit * 100
            if ratio >= desired_percent:
                filtered.append((name, count, moon_unit, steam_unit, ratio))

        total_moon_filtered = sum(moon_unit * count for _, count, moon_unit, _, _ in filtered)
        ...
        for name, count, moon_unit, steam_unit, ratio in filtered:
            prefix = f"{count}x " if count > 1 else ""
            print(f"{prefix}{name}: {moon_unit:.2f} / {steam_unit:.2f} = {ratio:.2f}%")
        print("---")
        if total_steam_filtered > 0:
            total_ratio_filtered = total_moon_filtered / total_steam_filtered * 100
        else:
            total_ratio_filtered = 0.0
        print(
            f"Total: {total_moon_filtered:.2f} / {total_steam_filtered:.2f} = {total_ratio_filtered:.2f}% / {filtered_count} items\n"
        )

    # 20 worst items by rate (lowest moon/steam %)
    if can_sell_grouped:
        ...
        for e in can_sell_grouped.values():
            ...
            if steam_unit <= 0:
                continue
            ratio = moon_unit / steam_unit * 100
            worst.append((name, count, moon_unit, steam_unit, ratio))

        worst.sort(key=lambda x: x[4])  # by ratio asc
        worst = worst[:20]

        print("20 worst items to sell on Moon (lowest rate):")
        worst_moon_total = sum(moon_unit * count for _, count, moon_unit, _, _ in worst)
        ...
        for name, count, moon_unit, steam_unit, ratio in worst:
            prefix = f"{count}x " if count > 1 else ""
            print(f"{prefix}{name}: {moon_unit:.2f} / {steam_unit:.2f} = {ratio:.2f}%")

        print("---")
        worst_ratio_total = (worst_moon_total / worst_steam_total * 100) if worst_steam_total > 0 else 0.0
        print(
            f"Total: {worst_moon_total:.2f} / {worst_steam_total:.2f} = {worst_ratio_total:.2f}% / {worst_count} items\n"
        )
